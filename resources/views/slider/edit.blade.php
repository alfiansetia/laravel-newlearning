        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="forms-sample" id="form" action="{{ route('slider.update', $data->id) }}" method="POST"
                        ...
                        <div class="form-group">
                            <textarea name="subtitle" id="summernote">{{ $data->subtitle }}</textarea>
                            @error('subtitle')
                                <div class="alert alert-danger mt-2">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <select name="show" id="show" class="form-control @error('show') is-invalid @enderror"
                                required>
                                <option {{ $data->show == 'yes' ? 'selected' : '' }} value="yes">Yes</option>
                                <option {{ $data->show == 'no' ? 'selected' : '' }} value="no">No</option>
                            </select>
                            @error('link')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="link" class="form-control @error('link') is-invalid @enderror"
                                id="link" placeholder="Link" value="{{ $data->link }}">
                            @error('link')
                                <div class="alert alert-danger mt-2">{{ $message }}</div>
                            @enderror
                        </div>
