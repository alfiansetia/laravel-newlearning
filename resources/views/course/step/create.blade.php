...
@section('content')
    <ul class="nav nav-pills nav-fill">
        <li class="nav-item">
            <a class="nav-link active" href="">Create Course</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="javascript:void(0);">Create Content</a>
        </li>
        <li class="nav-item">
                    ...
                    <div class="card-body">
                            ...
                            <select class="form-control @error('subcategory') is-invalid @enderror" name="subcategory"
                                ...
                                @foreach ($subcategories as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('subcategory') == $item->id ? 'selected' : '' }}>{{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('subcategory')
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="price" class="form-control @error('price') is-invalid @enderror"
                                id="price" placeholder="Price" value="{{ old('price', 1) }}" min="1" required>
                            @error('price')
                                <div class="alert alert-danger mt-2">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <textarea name="subtitle" id="subtitle">{{ old('subtitle') }}</textarea>
                            @error('subtitle')
                                <div class="alert alert-danger mt-2">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="header" class="form-control @error('header') is-invalid @enderror"
                                id="header" placeholder="header" value="{{ old('header') }}" required autofocus>
                            @error('header')
                                <div class="alert alert-danger mt-2">{{ $message }}</div>
                            @enderror
                        </div>
